import {Lazy, lazy, lift, sh, apply as applyLazy } from "./lazy"
import {Maybe, maybe, apply as applyMaybe, Nothing } from "./maybe"
...
export type LinkedList<T> = Lazy<Maybe<{
  head: T,
  tail: LinkedList<T>
}>>
...
export function take<T>(xs: LinkedList<T>, i:number): [LinkedList<T>, LinkedList<T>] {
  function rec(culm: LinkedList<T>, xs: LinkedList<T>, i:number): [LinkedList<T>, LinkedList<T>] {
    if (i <= 0) {
      return [culm, xs]
    }
...
export function lFold<T, U>(fn: (acc:U, v:T) => U, init: U, xs: LinkedList<T>): U {
  const list = lift(xs)
  if(list.type === "nothing") {
    return init
  }
...
const list = linkedList(lazy(1), linkedList(lazy(2), linkedList(lazy(3), linkedList(lazy(4), emptyLinkedList))))
console.log(list)
const timesTwo = map(list, (x) => applyLazy(x, (y) => { console.log("tick*2");  return y*2}) )
console.log(timesTwo)
const timesTwoAndFour = map(timesTwo, (x) => applyLazy(x, (y) => { console.log("tick*4");  return y*4}) )
